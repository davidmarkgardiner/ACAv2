...
if ! kubectl get eventbus default -n $NS &>/dev/null; then
    echo "ERROR: EventBus 'default' not found in namespace $NS"
    echo "Please deploy EventBus first"
    exit 1
fi
echo "   EventBus: OK"

if ! kubectl auth can-i create workflows --as=system:serviceaccount:$NS:argo-events-sa -n $NS &>/dev/null; then
    echo "ERROR: ServiceAccount argo-events-sa cannot create workflows"
    echo "Please apply RBAC first"
    exit 1
fi
echo "   RBAC: OK"
