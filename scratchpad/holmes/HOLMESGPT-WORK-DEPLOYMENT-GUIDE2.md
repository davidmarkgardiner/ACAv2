...
| **Azure OpenAI** | N/A | Yes | Same model (gpt-4o) works for Holmes |
...
| Azure OpenAI | Deployed model | gpt-4o or gpt-4o recommended |
...
This enhanced UI supports:
- **Cluster selection** dropdown (placeholder values)
- **Chat vs Investigate** mode toggle
- **Structured response display** with sections (Alert Explanation, Root Cause, Next Steps, etc.)
- **Namespace and Pod** fields for targeted investigations
...
data:
  index.html: |
    ...
    <head>
        ...
        <title>Holmes AI - K8s Investigation</title>
        <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .control-group label { font-size: 0.75rem; color: rgba(255,255,255,0.7); }
            ...
            .mode-btn:hover:not(.active) { border-color: rgba(255,255,255,0.5); }
            /* Chat Container */
            #chat-container {
                background: rgba(255,255,255,0.05);
                border-radius: 12px;
            ...
            .user-message { background: #0066ff; margin-left: 20%; }
            .assistant-message { background: rgba(255,255,255,0.1); margin-right: 10%; }
            .message-label { font-size: 0.75rem; opacity: 0.7; margin-bottom: 5px; }
            .message-content { white-space: pre-wrap; word-wrap: break-word; }
            /* Structured Response Sections */
            .response-sections { display: flex; flex-direction: column; gap: 12px; }
            .section-card {
            ...
            .section-card.alert { border-left-color: #ffc107; }
            .section-card.root-cause { border-left-color: #ff6b6b; }
            .section-card.actions { border-left-color: #28a745; }
            .section-card.team { border-left-color: #17a2b8; }
            .section-title {
                font-size: 0.8rem;
                font-weight: 600;
            ...
            .section-content { font-size: 0.95rem; line-height: 1.5; }
            /* Input Area */
            ...
            .examples h3 { font-size: 0.85rem; margin-bottom: 10px; color: rgba(255,255,255,0.7); }
            .example-btn {
                background: rgba(255,255,255,0.1);
                border: 1px solid rgba(255,255,255,0.2);
                padding: 8px 12px;
    ...
    <body>
        <header>
            <h1>Holmes AI - K8s Investigation</h1>
            <p>AI-powered Kubernetes troubleshooting with MCP</p>
        </header>
        <main>
            <!-- Controls -->
            <div class="controls">
                <div class="control-group">
                    ...
                    <select id="cluster-select">
                        <option value="aks-dev-001">aks-dev-001 (Dev)</option>
                        <option value="aks-staging-001">aks-staging-001 (Staging)</option>
                        <option value="aks-prod-001">aks-prod-001 (Prod)</option>
                        <option value="aks-prod-002">aks-prod-002 (Prod EU)</option>
                    </select>
                </div>
                <div class="control-group">
                    <label>Namespace</label>
                    <input type="text" id="namespace-input" placeholder="default" value="default" />
                </div>
                <div class="control-group">
                    <label>Pod/Resource (optional)</label>
                    <input type="text" id="pod-input" placeholder="e.g., nginx-abc123" />
                </div>
                <div class="control-group">
                    <label>Model</label>
                    <select id="model-select">
                        <option value="gpt-4o">gpt-4o (Fast)</option>
                        <option value="gpt-4o">gpt-4o (Detailed)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mode-toggle">
                <button class="mode-btn active" id="mode-chat" onclick="setMode('chat')">
                    💬 Chat Mode
                </button>
                <button class="mode-btn" id="mode-investigate" onclick="setMode('investigate')">
                    🔍 Investigate Mode
                </button>
            </div>
            ...
            <div id="chat-container">
                <div class="message assistant-message">
                    ...
                    <div class="message-content">Hello! I can help you investigate Kubernetes cluster issues.

<b>Chat Mode:</b> Ask general questions about your cluster
<b>Investigate Mode:</b> Get structured analysis with root cause and action items

Select a cluster and try asking about pods, deployments, or services.</div>
                </div>
            </div>

            <!-- Input -->
            <div id="input-container">
                <input type="text" id="question-input" placeholder="Describe the issue or ask a question..." />
            ...
            <div class="examples">
                ...
                <button class="example-btn" onclick="askExample(this)">Why is pod xyz crashing?</button>
                <button class="example-btn" onclick="askExample(this)">Check deployment health</button>
                <button class="example-btn" onclick="askExample(this)">Show recent error events</button>
                <button class="example-btn" onclick="askExample(this)">Investigate OOMKilled pods</button>
            </div>
        </main>
        ...
        <script>
            ...
            function setMode(mode) {
                ...
                document.getElementById('mode-investigate').classList.toggle('active', mode === 'investigate');
                input.placeholder = mode === 'chat'
                    ? 'Ask a question about your cluster...'
                    : 'Describe the issue to investigate (e.g., "Pod xyz crashing with OOMKilled")';
            }

            function addMessage(content, isUser, isStructured = false) {
            ...
            function formatStructuredResponse(data) {
                ...
                // If no structured sections, return plain analysis
                if (Object.keys(sections).length === 0 && analysis) {
                    return `<div class="section-card"><div class="section-content">${analysis}</div></div>`;
                }

                let html = '<div class="response-sections">';

                // Alert Explanation
                if (sections['Alert Explanation']) {
                    html += `
                        <div class="section-card alert">
                            <div class="section-title">📋 What Happened</div>
                            <div class="section-content">${sections['Alert Explanation']}</div>
                        </div>`;
                }
                ...
                if (sections['Key Findings']) {
                    html += `
                        <div class="section-card">
                            ...
                            <div class="section-content">${sections['Key Findings']}</div>
                        </div>`;
                }

                // Root Cause
                if (sections['Conclusions and Possible Root Causes']) {
                    html += `
                        <div class="section-card root-cause">
                            <div class="section-title">🎯 Root Cause</div>
                            <div class="section-content">${sections['Conclusions and Possible Root Causes']}</div>
                        </div>`;
                }

                // Team Assignment
                if (sections['App or Infra?']) {
                    html += `
                        <div class="section-card team">
                            <div class="section-title">👥 Team</div>
                            <div class="section-content">${sections['App or Infra?']}</div>
                        </div>`;
                }
                ...
                if (sections['Next Steps']) {
                    html += `
                        <div class="section-card actions">
                            ...
                            <div class="section-content">${sections['Next Steps']}</div>
                        </div>`;
                }

                // External Links
                if (sections['External links']) {
                    html += `
                        <div class="section-card">
                            <div class="section-title">📚 References</div>
                            <div class="section-content">${sections['External links']}</div>
                        </div>`;
                }

                // Fallback: show analysis if no sections matched
                if (html === '<div class="response-sections">' && analysis) {
                    html += `<div class="section-card"><div class="section-content">${analysis}</div></div>`;
                }
            ...
            async function sendMessage() {
                ...
                const namespace = document.getElementById('namespace-input').value || 'default';
                const pod = document.getElementById('pod-input').value;
                const model = document.getElementById('model-select').value;

                addMessage(`[${cluster}] ${question}`, true);
                input.value = '';
                sendBtn.disabled = true;

                const loadingDiv = document.createElement('div');
                loadingDiv.className = 'message assistant-message';
                loadingDiv.innerHTML = `<div class="loading"></div> ${currentMode === 'investigate' ? 'Investigating...' : 'Thinking...'}`;
                ...
                try {
                    ...
                    if (currentMode === 'investigate') {
                        // Use /api/investigate endpoint for structured response
                        ...
                        addMessage(formatStructuredResponse(data), false, true);

                    } else {
                        // Use /api/chat endpoint for simple chat
                        ...
                        chatContainer.removeChild(loadingDiv);
                        const result = data.analysis || data.response || data.result || JSON.stringify(data, null, 2);
...
data:
  default.conf: |
    server {
        ...
        # Proxy to Holmes /api/investigate endpoint (structured response)
        location /api/investigate {
            proxy_pass http://holmes-holmes.holmes.svc.cluster.local:80/api/investigate;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_read_timeout 300s;  # Investigations can take time
            proxy_connect_timeout 60s;
            proxy_send_timeout 60s;
        }

        # Proxy to Holmes /api/chat endpoint (simple chat)
        location /api/chat {
            proxy_pass http://holmes-holmes.holmes.svc.cluster.local:80/api/chat;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
- [ ] **Prerequisites**: Azure OpenAI deployed with gpt-4o model
