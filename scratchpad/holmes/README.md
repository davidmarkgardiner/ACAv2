...
mcp_servers:
  aks-mcp:
    ...
    llm_instructions: |
      ...
      | Friendly Name | Cluster Name           | Resource Group        |
      |---------------|------------------------|-----------------------|
      | prod          | aks-prod-westeurope    | rg-prod-westeurope    |
      | staging       | aks-staging-westeurope | rg-staging-westeurope |
      | dev           | aks-dev-westeurope     | rg-dev-westeurope     |

      ## MANDATORY WORKFLOW

      1. IDENTIFY the target cluster from the user's request
         - "prod", "production" → aks-prod-westeurope / rg-prod-westeurope
         - "staging", "stg" → aks-staging-westeurope / rg-staging-westeurope
         - "dev", "development" → aks-dev-westeurope / rg-dev-westeurope
         - If unclear, ASK the user which cluster

      2. GET CREDENTIALS using this exact tool call:
         Tool: az_aks_operations
         Parameters:
           operation: "get-credentials"
           args: "--name <cluster-name> --resource-group <resource-group> --overwrite-existing"

         Example for production:
           operation: "get-credentials"
           args: "--name aks-prod-westeurope --resource-group rg-prod-westeurope --overwrite-existing"

      3. VERIFY the context using kubectl:
         kubectl config current-context

      4. RUN your diagnostic kubectl commands

      ## RULES

      - NEVER guess which cluster - if ambiguous, ask
      - ALWAYS get credentials before running kubectl commands
