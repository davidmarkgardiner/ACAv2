...
./test-alerts.sh --context proxmox-k8s --all
```
...
|------|---------|-------------|
| **Webhook test (in-cluster, default)** | `./test-alerts.sh --context proxmox-k8s --webhook-test` | Sends a mock AlertManager payload from inside the cluster (no port-forward needed) |
| **Webhook test (local port-forward)** | `./test-alerts.sh --context proxmox-k8s --webhook-mode local --webhook-test` | Sends a mock payload to `localhost:12000` |
